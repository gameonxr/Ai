...
class ObservationBuilder:
    ...
    def build(self, physics_state: dict, timestamp: float) -> Observation:
        ...
        return Observation(timestamp=timestamp, proprioception=readings.get("proprioception"), vision=readings.get("vision"), depth=readings.get("depth"), segmentation=readings.get("segmentation"), perception=readings.get("perception"), imu=readings.get("imu"), touch=readings.get("touch"), info={"sensors_enabled": list(readings)})
