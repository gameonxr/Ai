...
The default configuration is `config/simulator_config.yaml`. It uses the MuJoCo backend boundary with a portable deterministic implementation for headless execution; the public backend interface is isolated so a native engine implementation can be introduced without changing the simulator or brain APIs. Rendering is disabled by default and can be enabled in YAML with `rendering.enabled: true`. The headless Matplotlib renderer auto-fits its viewport around the current body pose by default and draws a dashed ground reference at `ground_y: 0.0`; use `auto_scale: false` for the legacy fixed viewport, set a non-negative `padding` value to control the surrounding margin, set `show_ground: false` to hide the reference, or set `show_grid: true` to display a light coordinate grid for pose debugging.
...
`config/checkpoint_config.yaml` records the recommended checkpoint path and cadence. Checkpoints are emitted as `artifact_type: checkpoint` payloads with `schema_version: 1`, validated by artifact type, schema version, checkpoint version, and joint set before restoration, and written atomically so an interrupted save does not leave a partial checkpoint for a later resume. Older untyped checkpoints remain loadable because missing taxonomy fields default to `checkpoint` and schema version `1`; explicitly incompatible types or newer schema versions are rejected.
